"""
Proper scikit-fem cylinder flow simulation.

Wrapper class for cylinder flow simulation with three boundary conditions using
a real Navier-Stokes solver (not fake physics).
"""
...
class ProperSkfemCylinderFlow:
    """
    Cylinder flow simulation using proper scikit-fem Navier-Stokes solver.
    """
    ...
    def __init__(self, mesh_density: str = "medium", dt: float = 0.001,
        ...
        print(f"ProperSkfemCylinderFlow initialized:")
    ...
    def _generate_mesh(self):
        ...
        self.mesh_generator = SkfemMeshGenerator(
        ...
        print(f"  Mesh generated: {self.mesh.p.shape[1]} nodes, {self.mesh.t.shape[1]} elements")

    def _initialize_solver(self):
        """Initialize the proper Navier-Stokes solver."""
        print("Initializing proper Navier-Stokes solver...")

        self.solver = ProperSkfemNavierStokesSolver(
            mesh=self.mesh,
            nu=self.nu,
            rho=self.rho,
            dt=self.dt,
            reynolds_number=self.reynolds_number,
            debug=False  # Set to True for detailed step information
        )

        # Pass boundary information to solver
        if hasattr(self.mesh_generator, 'boundary_nodes'):
            self.solver.boundary_nodes = self.mesh_generator.boundary_nodes
        ...
        print("  Proper Navier-Stokes solver initialized")
    ...
    def run_simulation(self, max_steps: int = 1000, save_interval: int = 10,
        ...
        """
        print(f"\nRunning ProperSkfemCylinderFlow simulation...")
        ...
        for step in range(max_steps):
            ...
            # Print progress every 10 steps or at the beginning
            if step % 10 == 0 or step < 5:
                print(f"  Step {step+1}/{max_steps} (t={self.solver.simulation_time:.4f}s)")
            ...
            # Compute forces every 50 steps (reduced frequency for performance)
            if step % 50 == 0:
        ...
        print(f"\nProperSkfemCylinderFlow simulation completed:")
    ...
    def compute_pressure_drop(self) -> Tuple[float, float, float]:
        ...
        if not hasattr(self.mesh_generator, 'boundary_nodes'):
            return 0.0, 0.0, 0.0

        inlet_nodes = self.mesh_generator.boundary_nodes.get('inlet', [])
        outlet_nodes = self.mesh_generator.boundary_nodes.get('outlet', [])

        if not inlet_nodes or not outlet_nodes:
            return 0.0, 0.0, 0.0

        # Average pressure at inlet
        inlet_pressure = np.mean([self.solver.p[node] for node in inlet_nodes
                                 if node < len(self.solver.p)])

        # Average pressure at outlet
        outlet_pressure = np.mean([self.solver.p[node] for node in outlet_nodes
                                  if node < len(self.solver.p)])
    ...
    def save_results(self, results: Dict, filename: str):
        ...
        mesh_data = {
            'nodes': self.mesh.p.T,
            'elements': self.mesh.t.T,
            'boundary_nodes': getattr(self.mesh, 'boundaries', {}),
        }
        ...
        save_data = {
            ...
            'method': 'ProperSkfemFEM',
            'solver': 'proper_skfem_navier_stokes_solver'
        }

        # Save to compressed NumPy format
        np.savez_compressed(filename, **save_data)

        print(f"  ProperSkfemFEM results saved to: {filename}")
        print(f"  Saved {len(results.get('time', []))} time steps")
        print(f"  Mesh: {self.mesh.p.shape[1]} nodes, {self.mesh.t.shape[1]} elements")
...
def main():
    ...
    for condition in ["steady", "unsteady", "oscillating"]:
        ...
        # Save results
        simulation.save_results(results, f"results/proper_skfem/proper_skfem_{condition}_flow.npz")

        # Visualize
        simulation.visualize_solution(f"proper_skfem_{condition}_solution.png")
