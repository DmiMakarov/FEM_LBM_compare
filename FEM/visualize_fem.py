"""
Visualization tools for FEM cylinder flow results.
"""
...
class FEMVisualizer:
    """
    Visualization class for FEM cylinder flow results.
    """
    ...
    def __init__(self, results_file: str, fields_file: str = None):
        ...
        self.dt = float(self.results['dt'])
        self.nu = float(self.results['nu'])

        # Create coordinate arrays (assuming regular grid)
        # In practice, you'd want to use the actual mesh coordinates
        self.nx, self.ny = 100, 50
    ...
    def plot_strouhal_analysis(self, save_path: str = None) -> plt.Figure:
        ...
        if len(lift) > 100:
            ...
            freqs = np.fft.fftfreq(len(lift_detrended), d=np.mean(np.diff(time)))
            power = np.abs(fft)**2
    ...
    def generate_all_plots(self, output_dir: str = "fem_plots") -> None:
        """
...
def main():
    """Main function to visualize FEM results."""
    import glob

    # Find result files
    result_files = glob.glob("fem_results_Re*.npz")
    field_files = glob.glob("fem_results_Re*_fields.npz")
    ...
    for result_file in result_files:
        ...
        # Create visualizer
        visualizer = FEMVisualizer(result_file, field_file)
