"""
True proper FEM wrapper for cylinder flow simulation.
Uses the true proper FEM solver with correct shape functions and element assembly.
"""
...
class TrueCylinderFlowFEM:
    """
    True proper FEM simulation of 2D flow around a circular cylinder.
    ...
    def __init__(self, mesh_data_file: str = "meshes/small_mesh_20x10_data.npz",
        ...
        """
        Initialize true proper FEM cylinder flow simulation.
        ...
        mesh_dict = {
            ...
            'nx': mesh_data.get('nx', 20),
            'ny': mesh_data.get('ny', 10)
        }
    ...
    def run_simulation(self, max_steps: int = 1000, save_interval: int = 10,
        ...
        """
        Run true proper FEM simulation.
        ...
        """
        print(f"\nRunning true proper FEM simulation...")
        ...
        for step in range(max_steps):
            ...
            # Solve one time step using true proper FEM
            fem_start = time.time()
        ...
        print(f"\nTrue proper FEM simulation completed:")
    ...
    def save_results(self, results: Dict, filename: str):
        """
        Save FEM simulation results to disk.
        ...
        mesh_data = {
            ...
            'elements': self.fem.elements,
            'boundary_nodes': self.fem.boundary_nodes,
            'cylinder_nodes': self.fem.cylinder_nodes,
            'inlet_nodes': self.fem.inlet_nodes,
            'outlet_nodes': self.fem.outlet_nodes
        }

        # Get physical parameters
        physical_params = {
            'reynolds_number': self.fem.reynolds_number,
            'um': self.um,
            'dt': self.fem.dt,
            'nu': self.fem.nu,
            'rho': self.fem.rho,
            ...
            'domain_height': self.fem.domain_height,
            'initial_condition': self.fem.initial_condition
        }
        ...
        save_data = {
            ...
            # Metadata
            'n_nodes': len(self.fem.nodes),
            'n_elements': len(self.fem.elements),
            'method': 'FEM',
            'solver': 'true_fem_solver'
        }

        # Save to compressed NumPy format
        np.savez_compressed(filename, **save_data)

        print(f"  FEM results saved to: {filename}")
        print(f"  Saved {len(results.get('time', []))} time steps")
        print(f"  Mesh: {len(self.fem.nodes)} nodes, {len(self.fem.elements)} elements")
